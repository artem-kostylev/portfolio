...
export const CompanyCard = ({ company }: Props) => {
  return (
    <Card key={company.name}>
      <CardHeader className="p-4 items-center flex-row space-x-4">
        <Avatar className="w-11 h-11">
          <AvatarFallback>{company.name.charAt(0)}</AvatarFallback>
          <AvatarImage src={company.image} asChild>
            <Image src={company.image} alt={company.name} sizes="44px" fill />
          </AvatarImage>
        </Avatar>
        <div className="flex-1">
          <div className="flex items-center justify-between gap-2.5 mb-1">
            <CardTitle className="text-lg leading-3">{company.name}</CardTitle>
